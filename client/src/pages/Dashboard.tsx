...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  return (
    <div className="space-y-6 pb-12 font-sans text-slate-900 dark:text-zinc-100">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-white dark:bg-[#121216] border border-slate-200 dark:border-zinc-800 rounded-xl p-5 sm:p-6 shadow-sm font-mono transition-colors">
        <div>
          <div className="inline-flex items-center gap-2 px-2.5 py-0.5 rounded-md bg-slate-50 dark:bg-[#18181b] border border-slate-200 dark:border-zinc-800 text-blue-600 dark:text-blue-400 text-[10px] sm:text-xs font-bold uppercase tracking-wider mb-2">
            <span className="w-1.5 h-1.5 rounded-full bg-[#00d294]" />
            <span>Autonomous Intelligence Matrix</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-black font-display text-slate-900 dark:text-zinc-100 tracking-tight uppercase">
        ...
        <div className="flex items-center gap-3 bg-slate-50 dark:bg-[#18181b] border border-slate-200 dark:border-zinc-800 px-3.5 py-2.5 rounded-lg shrink-0">
          ...
          <div>
            <span className="text-xs font-bold text-slate-900 dark:text-zinc-100 block uppercase">Deduplication Vault</span>
      ...
      {topSignal && (
        <div className="bg-white dark:bg-[#121216] border border-slate-200 dark:border-zinc-800 rounded-xl p-5 sm:p-6 shadow-sm font-mono transition-colors">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-center">
            <div className="lg:col-span-2 space-y-2.5">
              <div className="flex flex-wrap items-center gap-1.5">
                <span className="text-xs font-bold px-2.5 py-0.5 rounded-md bg-slate-50 dark:bg-[#18181b] text-blue-600 dark:text-blue-400 border border-slate-200 dark:border-zinc-800 uppercase">
                  {topSignal.category}
                </span>
                {topSignal.aws_services.map(s => (
                  <span key={s} className="text-xs font-semibold px-2 py-0.5 rounded-md bg-slate-50 dark:bg-[#18181b] text-slate-700 dark:text-zinc-300 border border-slate-200 dark:border-zinc-800">
                    {s}
                  </span>
                ))}
              </div>
              ...
              <h2 
                ...
                className="text-base sm:text-xl font-bold text-slate-900 dark:text-zinc-100 hover:text-blue-600 dark:hover:text-blue-400 cursor-pointer transition-colors leading-snug font-sans"
              >
                {topSignal.title}
              </h2>
            ...
            <div className="bg-slate-50 dark:bg-[#18181b] border border-slate-200 dark:border-zinc-800 rounded-lg p-4 space-y-2.5 font-mono">
              <div className="text-center pb-2.5 border-b border-slate-200 dark:border-zinc-800">
                <span className="text-[10px] text-slate-500 dark:text-zinc-400 font-bold uppercase block tracking-wider">Bedrock Score</span>
                <span className="text-2xl font-black text-blue-600 dark:text-blue-400">{topSignal.signal_score}</span>
              ...
              <div className="space-y-1.5 text-xs">
                <div className="flex justify-between text-slate-600 dark:text-zinc-400">
                  ...
                  <strong className="text-slate-900 dark:text-zinc-100">{topSignal.importance_score}/100</strong>
                </div>
                <div className="flex justify-between text-slate-600 dark:text-zinc-400">
                  <span>Developer Value</span>
                  <strong className="text-slate-900 dark:text-zinc-100">{topSignal.relevance_score}/100</strong>
                </div>
                <div className="flex justify-between text-slate-600 dark:text-zinc-400">
                  <span>Momentum</span>
                  <strong className="text-slate-900 dark:text-zinc-100">{topSignal.momentum_score}/100</strong>
                </div>
              </div>
            </div>
          </div>
        ...
        {featuredTrend && (
          <div className="space-y-2.5">
            <div className="flex items-center justify-between font-mono">
              <h2 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-zinc-100 flex items-center gap-2 uppercase tracking-wide">
          ...
          <div className="space-y-2.5 font-mono">
            ...
            <div className="bg-white dark:bg-[#121216] border border-slate-200 dark:border-zinc-800 rounded-xl p-5 h-[calc(100%-2rem)] flex flex-col justify-between shadow-sm transition-colors">
              <div>
                ...
                <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-zinc-100 mb-1.5 leading-snug font-sans">
                  {briefing.try_today.title}
                </h3>
                <p className="text-slate-600 dark:text-zinc-400 text-xs leading-relaxed mb-3 font-sans">
      ...
      <div className="space-y-3 font-mono">
        <div className="flex items-center justify-between">
          <h2 className="text-sm sm:text-base font-bold text-slate-900 dark:text-zinc-100 uppercase tracking-tight">
            Recent Radar Signals
          </h2>
          <button 
            onClick={onExploreSignals}
            className="text-xs font-bold text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 uppercase"
          >
